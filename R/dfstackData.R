...
#' @import dplyr
#' @return The output from \code{\link{dfstackData}}
#' @export
dfstackData <- function(x,
  ...
  clone2 <- data.frame(matrix(1, nrow = 1,ncol = length(1:ncl_noxy)))
  ...
  d_gains <- cbind(timeIntervals,gainDf2)
  ...
  d_loss <- cbind(timeIntervals,lossDf2)

  if(!spatialextent %in% c('unified', 1)){
    lengthSpext <- lengthSpext * spatialextent
  }else {
    lengthSpext <- lengthSpext
  }
